...
const scriptPath = path.join(path.dirname(import.meta.url.replace('file:///', '')), 'lint_diagram.js');

test('lint_diagram fails on > 20 nodes', () => {
    const hugeDiagram = `graph TD\n${  Array.from({length: 25}, (_, i) => `A${i}-->B${i}`).join('\n')}`;
    const testFile = path.join(path.dirname(import.meta.url.replace('file:///', '')), 'test_huge.mmd');
