...
graph TD
    ...
    G1 -- Needs Plan --> RQ[request-plan]
    G1 -- Unverified Plan --> RC[receive-plan]
    ...
    TDD -- "spec ambiguous" --> RQ
```

## Transition States

- **TDD Escalation:** If TDD fails to pass after 3 attempts, it must return to `diagnose` or `request-plan`. This is now reflected directly in `SKILL.md`'s Gate 3 text.
