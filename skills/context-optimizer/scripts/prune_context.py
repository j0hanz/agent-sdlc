    ...
    if isinstance(data, dict):
        ...
        if not data:
            ...
            for idx, item in enumerate(data):
                ...
                lines.extend(to_markdown_kv(item, new_key))
    else:
...
def update_rolling_summary(
    ...
    for idx, entry in enumerate(existing_entries):
        ...
        if not lines:
            continue
        # Extract the timestamp from the first line (the session header line after "## Session: ")
        # e.g., "2026-06-21T10:00:00 (Current)" or "2026-06-21T10:00:00"
        ...
        if idx < 2:
            # Keep in full detail
            merged_history.append(f"## Session: {ts}\n{body}")
        else:
            ...
            for line in lines[1:]:
                if line.startswith("done:"):
                    ...
                    break
            merged_history.append(f"## Session: {ts} (Archived)\n- {done_line}")
...
def main() -> None:
    ...
    if args.logs:
        ...
        if not args.timestamp:
            sys.exit("error: --timestamp is required for updating the summary")
