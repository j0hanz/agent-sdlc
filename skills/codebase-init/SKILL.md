...
**Marker-detection gate:** Check for an existing root `AGENTS.md`. If it contains a `<!-- codebase-init:hard-rules v1 ... -->` marker comment matching the exact `v1` schema (anchor on the literal `v1` token — do not match other versions or treat the schema as version-agnostic), the survey was already completed — skip straight to Phase 1. If the marker is absent (including the case where `AGENTS.md` doesn't exist at all, or exists but was hand-authored or generated by the old `agents-maintainer` skill without a marker), proceed to ask the survey below.
